...
struct LuaManager::Impl {
    ...
    ScriptEntry* findScript(const std::string& name) {
        auto it = std::find_if(scripts.begin(), scripts.end(), [&](const ScriptEntry& script) { return script.name == name; });
    ...
    void bindApi() {
        ...
        ui.set_function("checkbox", [](const std::string& label, bool value) { ImGui::Checkbox(label.c_str(), &value); return value; });
        ui.set_function("slider_int", [](const std::string& label, int value, int minValue, int maxValue) { ImGui::SliderInt(label.c_str(), &value, minValue, maxValue); return value; });
        ui.set_function("slider_float", [](const std::string& label, float value, float minValue, float maxValue) { ImGui::SliderFloat(label.c_str(), &value, minValue, maxValue, "%.2f"); return value; });
        ...
        modules.set_function("is_enabled", [](const std::string& name) { Module* module = ModuleManager::i().getModuleByName(name); return module ? module->isToggled() : false; });
        modules.set_function("set_enabled", [](const std::string& name, bool enabled) {
            ...
            if (module->isToggled() != enabled) module->toggle();
            return true;
        });
        modules.set_function("get_key", [](const std::string& name) { Module* module = ModuleManager::i().getModuleByName(name); return module ? module->getKey() : 0; });
        modules.set_function("set_key", [](const std::string& name, int key) { Module* module = ModuleManager::i().getModuleByName(name); if (!module) return false; module->setKey(key); return true; });
    }

    bool callNoArgs(ScriptEntry& script, const char* callback) {
        if (!script.loaded || !script.environment) return false;
        sol::object object = (*script.environment)[callback];
        if (!object.valid() || object.get_type() != sol::type::function) return true;
    ...
    void unloadScript(ScriptEntry& script) {
        if (script.loaded) callNoArgs(script, "on_unload");
    ...
    void scanScripts() {
        ...
        if (selectedScript >= static_cast<int>(scripts.size())) selectedScript = scripts.empty() ? -1 : 0;
    }
#endif
};
...
void LuaManager::initialize() {
    ...
    impl->scanScripts();
    for (auto& script : impl->scripts) if (script.autoRun) impl->loadScript(script);
...
void LuaManager::shutdown() {
    if (!impl || !impl->initialized) return;
#if MENU_HAS_LUA
    for (auto& script : impl->scripts) impl->unloadScript(script);
    impl->scripts.clear();
#endif
...
void LuaManager::update(float deltaSeconds) {
    if (!impl || !impl->initialized) return;
#if MENU_HAS_LUA
    for (auto& script : impl->scripts) if (script.loaded) impl->callUpdate(script, deltaSeconds);
#else
    (void)deltaSeconds;
#endif
}
...
void LuaManager::renderMenu() {
    if (!impl) return;
    if (!impl->initialized) initialize();
    ImGui::TextUnformatted("Lua scripting");
    ImGui::SameLine();
    ImGui::TextDisabled(isAvailable() ? "Sol2 ready" : "disabled");
    ImGui::Spacing();
#if MENU_HAS_LUA
    if (ImGui::Button("Refresh scripts")) impl->scanScripts();
    ImGui::SameLine();
    if (ImGui::Button("Reload all")) for (auto& script : impl->scripts) if (script.loaded) impl->loadScript(script);
    ImGui::SameLine();
    ...
    for (int i = 0; i < static_cast<int>(impl->scripts.size()); ++i) {
        ...
        if (ImGui::Selectable(label.c_str(), impl->selectedScript == i)) impl->selectedScript = i;
    }
    ImGui::EndChild();
    ImGui::SameLine();
    ...
    if (impl->selectedScript >= 0 && impl->selectedScript < static_cast<int>(impl->scripts.size())) {
        ...
        if (ImGui::Checkbox("Auto-run on startup", &autoRun)) { script.autoRun = autoRun; impl->saveAutoRunList(); }
        if (!script.loaded) {
            if (ImGui::Button("Load")) impl->loadScript(script);
        } else {
            if (ImGui::Button("Reload")) impl->loadScript(script);
            ImGui::SameLine();
            if (ImGui::Button("Unload")) impl->unloadScript(script);
        }
    ...
    } else ImGui::TextDisabled("Select a .lua script from the list.");
    ImGui::EndChild();
#else
