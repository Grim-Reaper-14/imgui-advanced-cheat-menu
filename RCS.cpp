...
RCS::RCS() : Module(obf("RCS"), obf("Compensates for shooting recoil")) {
    standalone = &SetManager::i().add(new Set(true, obf("standalone"), getName())).getBVal();
    xPow = &SetManager::i().add(new Set(0.6f, 0.1f, 1.0f, obf("xPow"), getName())).getVec3f();
    yPow = &SetManager::i().add(new Set(0.5f, 0.1f, 1.0f, obf("yPow"), getName())).getVec3f();
}
...
void RCS::renderImGui() {
    ...
    ImGui::SliderFloat_(obf("Y Strength").c_str(), &yPow->x, yPow->y, yPow->z);
    ImGui::PopItemWidth();
}
